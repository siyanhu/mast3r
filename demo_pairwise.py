...
if __name__ == '__main__':
    ...
    for scn_dth in scene_dirs:
        ...
        for seq_dth in seq_dirs:
            ...
            for i in range (len(img_pths) - 1):
                if rt_count < demo_frames_limit[0]:
                    ...
                    ts_diff, im0name, im1name = pairwise(model, img_pths[i], img_pths[i + 1], check_vis=False, save_dir=pw_save_dir)
                    if ts_diff == -1:
                        continue
        ...
        fio.save_str_to_txt(str(rt_ts_diff_sum/rt_count), pw_ts_save_path, mode='a+')